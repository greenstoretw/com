...
const firebaseConfig = {
  apiKey: "AIzaSyBNYGv9KUJL15Hf9F3oD9ClJbYXvSzkKw0",
  authDomain: "loginback-3d50c.firebaseapp.com",
  projectId: "loginback-3d50c",
...
// --- Cloudflare Turnstile 網站金鑰 (請替換為您自己的網站金鑰) ---
const CLOUDFLARE_SITE_KEY = "0x4AAAAAABeXYd9f0btTQi-I"; // <-- 請替換為您的 Cloudflare Turnstile 網站金鑰

// --- Cloudflare Turnstile 密鑰 (!!! 警告：僅用於前端模擬，實際生產環境中必須在後端驗證 !!!) ---
// !!! 生產環境中，此密鑰絕不能暴露在前端程式碼中，否則會導致嚴重安全漏洞。
// !!! 實際情況下，您需要一個後端伺服器來安全地處理此驗證。
const CLOUDFLARE_SECRET_KEY = "0x4AAAAAABeXYQSXsR93pCSRa_8P4qBMeVM"; // <-- 實際生產環境中，此密鑰必須在後端使用！
...
const translations = {
    ...
    'de': {
        ...
        turnstileFailed: 'Menschliche Überprüfung fehlgeschlagen, bitte versuchen Sie es erneut.',
        dashboardTitle: 'Dashboard',
...
function Login({ setRoute, currentLang }) {
    ...
    const onLoginTurnstileSuccess = (token) => {
        setLoginTurnstileToken(token);
    };

    const onLoginTurnstileExpired = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // --- !!! 警告：前端的 Turnstile 密鑰驗證 (極度不安全) !!! ---
            // 實際生產環境中，此驗證必須在安全的後端伺服器上完成，
            // 且 CLOUDFLARE_SECRET_KEY 絕不能暴露在前端程式碼中。
            const turnstileVerificationResponse = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', {
                ...
                body: JSON.stringify({
                    secret: CLOUDFLARE_SECRET_KEY, // !!! 警告：生產環境中此密鑰絕不能在前端！
                    response: loginTurnstileToken,
                }),
            });
            const turnstileVerificationResult = await turnstileVerificationResponse.json();

            if (!turnstileVerificationResult.success) {
                setMessage(translations[currentLang].turnstileFailed);
                if (window.turnstile) {
                    window.turnstile.reset('#login-turnstile-widget');
                    setLoginTurnstileToken(null);
    ...
    // 確保 Turnstile 腳本載入後渲染小工具
    useEffect(() => {
        // 檢查 window.turnstile 是否已定義，因為腳本是異步載入的
        const renderTurnstile = () => {
            if (window.turnstile) {
                window.turnstile.render('#login-turnstile-widget', {
                    sitekey: CLOUDFLARE_SITE_KEY,
                    callback: onLoginTurnstileSuccess,
                    'expired-callback': onLoginTurnstileExpired,
                });
            } else {
                // 如果 Turnstile 腳本還沒載入，等待它
    ...
    return (
        <section id="login" className="py-12 bg-gray-100 min-h-screen flex items-center justify-center">
            <div className="container mx-auto px-4">
                <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg p-8">
                    <h2 className="text-3xl font-bold text-center mb-6">{translations[currentLang].loginTitle}</h2>
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="flex items-center justify-between">
                            <button type="submit" className="bg-green-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-green-700 transition">{translations[currentLang].loginBtn}</button>
                            <button 
                                type="button" 
                                onClick={() => setRoute('register')} 
                                className="inline-block align-baseline font-bold text-sm text-green-600 hover:text-green-800"
                            >
                                {translations[currentLang].noAccountYet} {translations[currentLang].goToRegister}
                            </button>
                        </div>
                    </form>
                </div>
...
function Register({ setRoute, currentLang }) {
    ...
    return (
        <section id="register" className="py-12 bg-gray-100 min-h-screen flex items-center justify-center">
            <div className="container mx-auto px-4">
                <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg p-8">
                    <h2 className="text-3xl font-bold text-center mb-6">{translations[currentLang].registerTitle}</h2>
                    <form onSubmit={handleSubmit}>
                        <div className="mb-4">
                            <label htmlFor="email" className="block text-gray-700 text-sm font-bold mb-2">{translations[currentLang].emailLabel}</label>
                            <input 
                        ...
                        <div className="flex items-center justify-between">
                            <button type="submit" className="bg-green-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-green-700 transition">{translations[currentLang].registerBtn}</button>
                            <button 
                                type="button" 
                                onClick={() => setRoute('login')} 
                                className="inline-block align-baseline font-bold text-sm text-green-600 hover:text-green-800"
                            >
                                {translations[currentLang].alreadyHaveAccount} {translations[currentLang].goToLogin}
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <section id="dashboard" className="py-12 bg-green-50 min-h-screen">
            <div className="container mx-auto px-4">
                <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-8">
                    <h2 className="text-3xl font-bold text-center mb-4">{translations[currentLang].dashboardTitle}</h2>
                    <p className="text-xl text-gray-700 mb-4 text-center">
                        {translations[currentLang].welcomeMessage}
                        <span className="font-semibold">{currentUser ? currentUser.email : '訪客'}</span>!
                    </p>
                    <p className="text-gray-600 mb-8 text-center">{translations[currentLang].protectedContent}</p>
                    
                    {/* 新增商店表單 */}
                    <div className="mb-12">
                        <h3 className="text-2xl font-bold mb-4">{translations[currentLang].addStoreTitle}</h3>
                        <form onSubmit={handleAddStore} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="md:col-span-2 text-right">
                                <button type="submit" className="btn-primary px-6 py-2 rounded-lg font-medium">{translations[currentLang].addStoreBtn}</button>
                            </div>
                        </form>
                    </div>
                    ...
                    {/* 現有商店列表 */}
                    <div>
                        <h3 className="text-2xl font-bold mb-4">{translations[currentLang].storeListTitle}</h3>
                        {stores.length === 0 ? (
                            <p className="text-gray-600 text-center">{translations[currentLang].noStoresYet}</p>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {stores.map(store => (
                                    <div key={store.id} className="bg-green-100 p-4 rounded-lg shadow-sm">
        ...
        <nav className="bg-white shadow-md sticky top-0 z-50">
            <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                <div className="flex items-center space-x-2">
                    ...
                    <a href="#" className="text-2xl font-bold text-gray-800" onClick={() => setRoute('login')}>{translations[currentLang].appName}</a>
                </div>
                <div className="hidden md:flex space-x-6 items-center">
                    <button onClick={() => setRoute('login')} className="text-gray-700 hover:text-green-600 transition">{translations[currentLang].login}</button>
                    <button onClick={() => setRoute('register')} className="text-gray-700 hover:text-green-600 transition">{translations[currentLang].register}</button>
                    <select id="language-select" onChange={handleLanguageChange} value={currentLang} className="bg-green-500 text-white text-sm px-3 py-1 rounded-full hover:bg-green-700 transition cursor-pointer">
                ...
                <div className="md:hidden flex items-center">
                    <select id="language-select-mobile" onChange={handleLanguageChange} value={currentLang} className="bg-green-500 text-white text-sm px-3 py-1 rounded-full hover:bg-green-700 transition cursor-pointer mr-2">
                        <option value="zh-TW">{translations['zh-TW'].langChinese}</option>
                        <option value="en">{translations['en'].langEnglish}</option>
                        <option value="fr">{translations['fr'].langFrench}</option>
                        <option value="de">{translations['de'].langGerman}</option>
            ...
            <div id="mobile-menu" className="hidden md:hidden px-4 py-2 bg-white">
                <button onClick={() => setRoute('login')} className="block py-2 text-gray-700 hover:text-green-600 w-full text-left">{translations[currentLang].login}</button>
                <button onClick={() => setRoute('register')} className="block py-2 text-gray-700 hover:text-green-600 w-full text-left">{translations[currentLang].register}</button>
            </div>
        </nav>
    );
